...
class LoadSingleSentenceClassificationDataset:
    ...
    def data_process(self, file_path=None):
        ...
        return data, max_len, sequences

    def load_train_val_test_data(self, train_file_path=None,
                                 val_file_path=None,
                                 test_file_path=None,
                                 only_test=False):
        test_data, _, _ = self.data_process(file_path=test_file_path)
        test_iter = DataLoader(test_data, batch_size=self.batch_size,
                               shuffle=False, collate_fn=self.generate_batch)
        if only_test:
            return test_iter
        train_data, max_sen_len, train_sequences = self.data_process(file_path=train_file_path)  # 得到处理好的所有样本
        if self.max_sen_len == 'same':
            self.max_sen_len = max_sen_len
        val_data, _, _ = self.data_process(file_path=val_file_path)
        train_iter = DataLoader(train_data, batch_size=self.batch_size,  # 构造DataLoader
                                shuffle=self.is_sample_shuffle, collate_fn=self.generate_batch)
        val_iter = DataLoader(val_data, batch_size=self.batch_size,
                              shuffle=False, collate_fn=self.generate_batch)
        sequence_iter = DataLoader(train_sequences, batch_size=self.batch_size,
                                   shuffle=True, collate_fn=self.generate_batch)
        return train_iter, test_iter, val_iter, sequence_iter
...
class Temp(LoadSingleSentenceClassificationDataset):
...
class LoadPairSentenceClassificationDataset(Temp):
    def __init__(self, **kwargs):
...
class LoadMultipleChoiceDataset(Temp):
...
class LoadSQuADQuestionAnsweringDataset(Temp):
    """
    Args:
...
class LoadChineseNERDataset(Temp):
