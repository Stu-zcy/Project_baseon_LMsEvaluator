...
logging.info(args.visible_device)
# print(f'args.visible_device: {args.visible_device}')
...
weights = []
logging.info(seed)
logging.info(args.query_num)
# print(f'seed: {seed}')
# print(f'args.query_num: {args.query_num}')
...
def train_steal_model(steal_train, steal_val):
    ...
    logging.info(f"steal model: {args.steal_model_version}")
    ...
    logging.info('training steal models on query dataset...')
    ...
    for epoch in range(Epochs):
        ...
        logging.info(f'Epoch [{epoch + 1}/{Epochs}],  train_loss: {tloss:.4f}, train_acc: {tacc:.4f}, vali_acc: {vacc:.4f}')
    ...
    #     logging.info('testing steal models on query dataset...')
    if args.steal_model_version == 'bert_base_uncased':
        model = BPC(args)
    elif args.steal_model_version == 'roberta_base':
        model = RPC(args)
...
def calculate_acc_agreement(steal_model, victim_model, original_test_data):
    ...
    logging.info(f"Evaluation Result on {args.task_name}: victim acc {vac:.4f}, steal acc {acc:.4f}, agreement {agreement:.4f}")
    # print("seed: ", seed)
...
def main():
    ...
    logging.info(f"vic model: {args.victim_model_version}")
    ...
    if vck[0] != '-':
        vck = '-' + vck
    checkpoint = torch.load(args.saved_model_path + args.task_name + vck, map_location=device, weights_only=True)
