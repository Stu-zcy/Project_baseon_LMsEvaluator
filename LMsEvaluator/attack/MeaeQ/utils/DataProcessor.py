...
class SST2DataProcessor:
    def __init__(self, args):
        ...
        self.SST2_data_path = os.path.join(args.project_root_path, 'datasets', 'MeaeQ', 'SST-2')
    ...
    def read_data(self, flag):
        ...
        #     data = f.read()
        with open(os.path.join(self.SST2_data_path, self.data_file_list[idx]), 'r') as f:
            data = f.read()
        lines = data.split('\n')
        sentence = []
        label = []
...
class IMDBDataProcessor:
    def __init__(self, args):
        ...
        # args.project_NLP_path = /LMsEvaluator/attack/MeaeQ
        self.IMDB_data_path = os.path.join(args.project_root_path, 'datasets', 'MeaeQ', 'aclImdb')
        self.IMDB_data_save_dir = os.path.join(args.project_root_path, 'datasets', 'MeaeQ', 'aclImdb', 'data')
        self.train_file = os.path.join(self.IMDB_data_save_dir, 'imdb_train.npz')
        self.val_file = os.path.join(self.IMDB_data_save_dir, 'imdb_val.npz')
        self.test_file = os.path.join(self.IMDB_data_save_dir, 'imdb_test.npz')
        self.imdb_output_path = args.imdb_path

    def get_data(self, data_path):
        pos_files = os.listdir(os.path.join(data_path, 'pos'))
        neg_files = os.listdir(os.path.join(data_path, 'neg'))
        print(len(pos_files))
        print(len(neg_files))
        ...
        for pf, nf in zip(pos_files, neg_files):
            ...
            #     s = f.read()
            #     neg_all.append(s)
            with open(os.path.join(data_path, 'pos', pf), encoding='utf-8') as f:
                s = f.read()
                pos_all.append(s)
            with open(os.path.join(data_path, 'neg', nf), encoding='utf-8') as f:
                s = f.read()
                neg_all.append(s)
    ...
    def generate_train_data(self):
        X_orig, Y_orig = self.get_data(os.path.join(self.IMDB_data_path, 'train'))
        X_test, Y__test = self.get_data(os.path.join(self.IMDB_data_path, 'test'))
        ...
        np.savez(os.path.join(self.IMDB_data_save_dir, 'imdb_train'), x=X_train, y=y_train)
        np.savez(os.path.join(self.IMDB_data_save_dir, 'imdb_test'), x=X_test, y=y_test)
        np.savez(os.path.join(self.IMDB_data_save_dir, 'imdb_val'), x=X_val, y=y_val)
...
class AGNEWSDataProcessor:
    def __init__(self, args=None):
        ...
        self.AGNEWS_data_path = os.path.join(args.project_root_path, 'datasets', 'MeaeQ', 'ag_news')
    ...
    def read_data(self, flag):
        ...
        with open(os.path.join(self.AGNEWS_data_path, self.data_file_list[idx]), 'r') as f:
            f_csv = csv.reader(f)
            headers = next(f_csv)
...
class HATESPEECHDataProcessor:
    def __init__(self, args=None):
        ...
        self.HATESPEECH_data_path = os.path.join(args.project_root_path, 'datasets', 'MeaeQ', 'hate_speech')
        ...
        with open(os.path.join(self.HATESPEECH_data_path, self.label_file), 'r', encoding='utf-8') as f:
            f_csv = csv.reader(f)
            headers = next(f_csv)
            for row in f_csv:
        ...
        test_sentence = []
        test_label = []
        for file_name in os.listdir(os.path.join(self.HATESPEECH_data_path, self.data_file_list[2])):
            test_file_name.append(file_name)
            # with open(self.HATESPEECH_data_path + self.data_file_list[2] + '/' + file_name, 'r', encoding='utf-8') as f:
            #     data = f.readlines()
            with open(os.path.join(self.HATESPEECH_data_path, self.data_file_list[2], file_name), 'r',
                      encoding='utf-8') as f:
                data = f.readlines()
        ...
        val_sentence = []
        val_label = []
        for file_name in os.listdir(os.path.join(self.HATESPEECH_data_path, self.data_file_list[1])):
            test_file_name.append(file_name)
            # with open(self.HATESPEECH_data_path + self.data_file_list[1] + '/' + file_name, 'r', encoding='utf-8') as f:
            #     data = f.readlines()
            with open(os.path.join(self.HATESPEECH_data_path, self.data_file_list[1], file_name), 'r',
                      encoding='utf-8') as f:
                data = f.readlines()
