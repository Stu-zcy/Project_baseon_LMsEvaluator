...
class MyTextAttack(BaseAttack):
    def __init__(self, config_parser, attack_config, model=None, tokenizer=None, use_local_model=False,
                 use_local_tokenizer=False, use_local_dataset=False, model_name_or_path=None,
                 tokenizer_name_or_path=None, dataset_name_or_path=None, display_full_info=False, defender=None):
    ...
    def attack(self):
        ...
        #     self.model = transformers.AutoModelForSequenceClassification.from_pretrained(self.model_name_or_path)
        # if self.tokenizer is None:
        #     self.tokenizer = transformers.AutoTokenizer.from_pretrained(self.tokenizer_name_or_path)
        # self.model = transformers.AutoModelForSequenceClassification.from_pretrained(self.model_name_or_path,
        #                                                                              local_files_only=True)
        # self.model = model_load(base_model=self.model, map_location=next(self.model.parameters()).device,
        #                         model_dir=os.path.join('./', self.config_parser['LM_config']['model_save_dir']))
        # self.tokenizer = transformers.AutoTokenizer.from_pretrained(self.tokenizer_name_or_path)
        ...
        if self.use_local_dataset:
            ...
            with open(self.dataset_name_or_path, 'r', encoding='utf-8') as file:
                ...
                for data_line in data_lines:
                    ...
                    temp_label = data_line_split[-1]
                    if 'ChineseNER' in self.dataset_name_or_path:
                        entities = {'O': 0, 'B-ORG': 1, 'B-LOC': 2, 'B-PER': 3, 'I-ORG': 4, 'I-LOC': 5, 'I-PER': 6}
                        if '\n' in temp_label:
                            temp_label = temp_label.replace('\n', '')
                        temp_label = entities[temp_label]
                    local_dataset.append((temp, int(temp_label)))
